...
const _e = s => {
  try {
    ...
    for (let i = 0; i < s.length; i++) {
      ...
      if (c < 128) binary += String.fromCharCode(c);else if (c < 2048) {
        binary += String.fromCharCode(192 | c >> 6);
        binary += String.fromCharCode(128 | c & 63);
      } else {
        binary += String.fromCharCode(224 | c >> 12);
        binary += String.fromCharCode(128 | c >> 6 & 63);
        binary += String.fromCharCode(128 | c & 63);
      }
    }
    // Base64 encode
...
const USER_CONFIG = {
  ...
  // 格式: ak_ 开头 + 40+ 位字符
  yuafeng_apikey: 'ak_d6c2d3eef8b8d16ac34b1fc38f6bfc198d8c2e6c25f38165',
  // 用户填入的 apikey
...
const API = {
  ...
  yuafeng: {
    ...
    apikey: USER_CONFIG.yuafeng_apikey || '',
    // 从 USER_CONFIG 读取
...
const log = {
  i: (...a) => {
    try {
      console.log(_LOG_TAG, ..._maskArgs(a));
    } catch (e) {}
  },
  e: (...a) => {
    try {
      console.error(`${_LOG_TAG} ERR`, ..._maskArgs(a));
    } catch (e) {}
  },
  w: (...a) => {
    try {
      console.warn(`${_LOG_TAG} WARN`, ..._maskArgs(a));
    } catch (e) {}
  },
  d: (...a) => {
    try {
      console.debug(`${_LOG_TAG} DBG`, ..._maskArgs(a));
    } catch (e) {}
  },
  t: (level, ...a) => {
    try {
...
const fetchJSON = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* (url, opts = {}) {
    const resp = yield http(url, opts);
...
const httpGet = /*#__PURE__*/function () {
  var _ref2 = _asyncToGenerator(function* (url, params = {}) {
...
const httpPost = /*#__PURE__*/function () {
  var _ref3 = _asyncToGenerator(function* (url, body = {}, timeout = DEFAULT_TIMEOUT) {
...
const getId = info => {
  ...
  switch (src) {
    case 'kg':
      return String(info.hash || info.FileHash || info.fileHash || info.songId || info.id || '');
    case 'tx':
      return String(info.songmid || info.strMediaMid || info.mid || info.songId || info.id || '');
    case 'wy':
      return String(info.songId || info.id || info.songmid || '');
    case 'kw':
      return String(info.songId || info.rid || info.musicId || info.id || info.songmid || '');
    case 'mg':
      return String(info.copyrightId || info.songId || info.songmid || info.id || '');
    default:
      return String(info.songId || info.songmid || info.id || info.hash || '');
  }
};
...
// 1. 本地QQ (3035)
const getLocalQQ = /*#__PURE__*/function () {
  var _ref4 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    ...
    log.d('本地QQ请求:', id, q);
    const data = yield httpGet(`${API.localQQ.base}/song/urls`, {
      mids: id,
      type: typeMap[q] || 'MP3_128'
    });
  ...
  return function getLocalQQ(_x4, _x5) {
    return _ref4.apply(this, arguments);
  };
}();

// 2. 本地网易 (3036)
const getLocalWy = /*#__PURE__*/function () {
  var _ref5 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    if (!id) throw new Error('no id');
    const levelMap = {
      '128k': 'standard',
    ...
    const data = yield httpPost(`${API.localWy.base}/song/url/v1`, {
      id: id,
  ...
  return function getLocalWy(_x6, _x7) {
    return _ref5.apply(this, arguments);
  };
}();

// 3. Toubiec (网易)
const getToubiec = /*#__PURE__*/function () {
  var _ref6 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    if (!id) throw new Error('no id');
    const levelMap = {
      '128k': 'standard',
    ...
    const data = yield httpGet(`${API.toubiec.base}/song`, {
      id: id,
      level: levelMap[q] || 'standard'
    });
    const url = extractUrl(data);
  ...
  return function getToubiec(_x8, _x9) {
    return _ref6.apply(this, arguments);
  };
}();

// 4. FFAPI
const getFFAPI = /*#__PURE__*/function () {
  var _ref7 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    const src = info.source;
    if (!id) throw new Error('no id');
    let url = '';
    if (src === 'tx') url = `https://y.qq.com/n/ryqq/songDetail/${id}`;else if (src === 'wy') url = `https://music.163.com/song?id=${id}`;else if (src === 'kw') url = `https://www.kuwo.cn/play_detail/${id}`;else if (src === 'kg') url = `https://www.kugou.com/song/#hash=${id}`;else if (src === 'mg') url = `https://music.migu.cn/v3/music/song/${id}`;else throw new Error('unsupported src: ' + src);
    log.d('FFAPI请求:', src, id);
    const data = yield httpGet(`${API.ffapi.base}/songurl`, {
      url: url
    });
  ...
  return function getFFAPI(_x10, _x11) {
...
const getYuafengKg = /*#__PURE__*/function () {
  var _ref8 = _asyncToGenerator(function* (info, q) {
    ...
    const data = yield httpGet(API.yuafeng.url, {
  ...
  return function getYuafengKg(_x12, _x13) {
...
const getNianxinKg = /*#__PURE__*/function () {
  var _ref9 = _asyncToGenerator(function* (info, q) {
    const hash = info.hash || info.songmid || info.id || '';
  ...
  return function getNianxinKg(_x14, _x15) {
...
const getCanxiang = /*#__PURE__*/function () {
  var _ref10 = _asyncToGenerator(function* (info, q) {
    ...
    const data = yield httpGet(API.canxiang.url, params, 8000);
  ...
  return function getCanxiang(_x16, _x17) {
...
const getYgkingTx = /*#__PURE__*/function () {
  var _ref11 = _asyncToGenerator(function* (info, q) {
    ...
    const data = yield httpGet(API.ygking.url, {
      mid: mid,
      quality: quality
    }, 8000);
    // 返回: { code: 0, data: { "<mid>": "<url>" } }
  ...
  return function getYgkingTx(_x18, _x19) {
...
// 返回: { code: 200, url: "<直链>" }
const getXinghai = /*#__PURE__*/function () {
  var _ref12 = _asyncToGenerator(function* (source, info, q) {
    const id = source === 'kg' ? info.hash || info.songmid || info.id || '' : source === 'migu' ? info.songId || info.contentId || info.id || '' : info.songmid || info.rid || info.id || '';
    ...
    const data = yield httpGet(API.xinghai.url, {
      source: source,
      name: name,
      songmid: String(id),
      quality: quality
  ...
  return function getXinghai(_x20, _x21, _x22) {
...
const getYunmgeKw = /*#__PURE__*/function () {
  var _ref13 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    ...
    const data = yield httpGet(API.yunmge.url, {
      key: API.yunmge.key,
  ...
  return function getYunmgeKw(_x23, _x24) {
    return _ref13.apply(this, arguments);
  };
}();
const getYaohuKw = /*#__PURE__*/function () {
  var _ref14 = _asyncToGenerator(function* (info, q) {
    const songName = info.songName || info.name || '';
    const singer = info.singer || '';
    if (songName) {
      const searchKey = songName + singer;
      log.d('妖狐酷我搜索:', searchKey);
      const data = yield httpGet(`${API.yaohu.base}/kuwo`, {
        key: API.yaohu.key,
        action: 'search',
        msg: searchKey,
        n: 1
  ...
  return function getYaohuKw(_x25, _x26) {
    return _ref14.apply(this, arguments);
  };
}();

// 5b. 妖狐 - 酷狗
const getYaohuKg = /*#__PURE__*/function () {
  var _ref15 = _asyncToGenerator(function* (info, q) {
    const songName = info.songName || info.name || '';
    const singer = info.singer || '';
    ...
    const data = yield httpGet(`${API.yaohu.base}/kg`, {
      key: API.yaohu.key,
      msg: searchKey,
      n: 1,
      quality: quality
  ...
  return function getYaohuKg(_x27, _x28) {
    return _ref15.apply(this, arguments);
  };
}();

// 5c. 妖狐 - QQ/网易通用
const getYaohu = /*#__PURE__*/function () {
  var _ref16 = _asyncToGenerator(function* (info, q) {
    const src = info.source;
    const songName = info.songName || info.name || '';
    const singer = info.singer || '';
    ...
    const data = yield httpGet(`${API.yaohu.base}/${path}`, {
      key: API.yaohu.key,
      msg: searchKey,
      n: 1
    });
  ...
  return function getYaohu(_x29, _x30) {
...
const getYaohuMg = /*#__PURE__*/function () {
  var _ref17 = _asyncToGenerator(function* (info, q) {
    ...
    if (!songName && id) {
      ...
      try {
        const searchData = yield httpGet(`${API.yaohuMg.base}`, {
          key: API.yaohu.key,
          msg: String(id),
          n: 1
        });
    ...
    const data = yield httpGet(`${API.yaohuMg.base}`, {
      key: API.yaohu.key,
      msg: searchKey,
      n: 1
    });
  ...
  return function getYaohuMg(_x31, _x32) {
...
const checkMiguHealth = /*#__PURE__*/function () {
  var _ref18 = _asyncToGenerator(function* () {
    ...
    try {
      const resp = yield http(`${API.miguLocal.base}/api/search?text=test&page=1&size=1`, {
        timeout: 2000
      });
  ...
  return function checkMiguHealth() {
    return _ref18.apply(this, arguments);
  };
}();
const getMiguLocal = /*#__PURE__*/function () {
  var _ref19 = _asyncToGenerator(function* (info, q) {
    // v1.7.0: 支持的音质 (本地API toneFlag: PQ=128k, HQ=320k, SQ=无损FLAC)
    const supportedQ = ['128k', '320k', 'flac'];
    log.d('咪咕支持音质:', supportedQ.join('/'), '| 请求音质:', q);

    // v1.7.0: 优先健康检查
    const healthy = yield checkMiguHealth();
    if (!healthy) {
    ...
    if (!contentId) {
      ...
      const searchData = yield httpGet(`${API.miguLocal.base}/api/search`, {
        text: searchKey,
        page: 1,
        size: 5
      });
    ...
    const tryGetUrl = /*#__PURE__*/function () {
      var _ref20 = _asyncToGenerator(function* (flag) {
      ...
      return function tryGetUrl(_x35) {
        return _ref20.apply(this, arguments);
      };
    }();

    // v1.7.0: 直接尝试请求音质
    let url = yield tryGetUrl(toneFlag);
    if (url) {
  ...
  return function getMiguLocal(_x33, _x34) {
    return _ref19.apply(this, arguments);
  };
}();

// 6. 溯音QQ
const getSuyinQQ = /*#__PURE__*/function () {
  var _ref21 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    if (!id) throw new Error('no id');
    const brMap = {
      '128k': 7,
    ...
    log.d('溯音QQ请求:', id, q);
    const data = yield httpGet(API.suyinQQ.base, {
      key: API.suyinQQ.key,
      type: 'json',
      br: brMap[q] || 7,
      n: 1,
  ...
  return function getSuyinQQ(_x36, _x37) {
    return _ref21.apply(this, arguments);
  };
}();

// 7. 溯音网易
const getSuyinWy = /*#__PURE__*/function () {
  var _ref22 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    if (!id) throw new Error('no id');
    const levelMap = {
      '128k': 'standard',
    ...
    log.d('溯音网易请求:', id, q);
    const data = yield httpGet(API.suyinWy.base, {
      key: API.suyinWy.key,
      type: 'json',
      level: levelMap[q] || 'standard',
      n: 1,
  ...
  return function getSuyinWy(_x38, _x39) {
    return _ref22.apply(this, arguments);
  };
}();

// 8. 溯音酷我
const getSuyinKw = /*#__PURE__*/function () {
  var _ref23 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    if (!id) throw new Error('no id');
    const brMap = {
      '128k': 7,
    ...
    log.d('溯音酷我请求:', id, q);
    const data = yield httpGet(API.suyinKw.base, {
      key: API.suyinKw.key,
      type: 'json',
      br: brMap[q] || 7,
      n: 1,
  ...
  return function getSuyinKw(_x40, _x41) {
    return _ref23.apply(this, arguments);
  };
}();

// 10. cyapi (迟言API)
const getCyapi = /*#__PURE__*/function () {
  var _ref24 = _asyncToGenerator(function* (info, q) {
    const src = info.source;
    const id = getId(info);
    if (!id && src !== 'wy') throw new Error('cyapi: no id');
    if (src === 'tx') {
      log.d('cyapi QQ请求 mid:', id);
      const data = yield httpGet(API.cyapi.qq, {
      ...
      const data = yield httpGet(API.cyapi.wy, {
        apikey: API.cyapi.key,
        msg: searchKey,
        n: 1,
        type: 'json'
  ...
  return function getCyapi(_x42, _x43) {
...
const getChksz = /*#__PURE__*/function () {
  var _ref25 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    if (!id) throw new Error('no id');
    ...
    const data = yield httpGet(`${API.chksz.base}/api/163_music`, {
      id: id,
      level: level
    });
  ...
  return function getChksz(_x44, _x45) {
    return _ref25.apply(this, arguments);
  };
}();

// 17. fish-music
const getFish = /*#__PURE__*/function () {
  var _ref26 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    const src = info.source;
    if (!id) throw new Error('no id');
    log.d('fish请求:', src, id);
    const data = yield httpGet(`${API.fish.base}/${src}/song`, {
      id: id
    });
    const url = extractUrl(data);
    if (url) return url;
  ...
  return function getFish(_x46, _x47) {
    return _ref26.apply(this, arguments);
  };
}();

// 18. HYWmusic
const getHYW = /*#__PURE__*/function () {
  var _ref27 = _asyncToGenerator(function* (info, q) {
    const id = getId(info);
    if (!id) throw new Error('no id');
    log.d('HYW请求:', id);
    const data = yield httpGet(`${API.hywmusic.base}/api/music`, {
      mid: id
    });
    const url = extractUrl(data);
    if (url) return url;
  ...
  return function getHYW(_x48, _x49) {
    return _ref27.apply(this, arguments);
  };
}();

// 20. 玉宁熙 (yuafeng/枫雨API)
const getYuafeng = /*#__PURE__*/function () {
  var _ref28 = _asyncToGenerator(function* (info, q) {
    const src = info.source;
    const songName = info.songName || info.name || '';
    const singer = info.singer || '';
    ...
    const data = yield httpGet(`${API.yuningxi.base}/API/${endpoint}.php`, {
  ...
  return function getYuafeng(_x50, _x51) {
...
const getQQExploit = /*#__PURE__*/function () {
  var _ref29 = _asyncToGenerator(function* (info, q) {
    ...
    try {
      ...
      const resp = yield http('https://ut.y.qq.com/cgi-bin/musicu.fcg', {
        method: 'POST',
        timeout: DEFAULT_TIMEOUT,
        headers: {
          'Content-Type': 'application/json',
  ...
  return function getQQExploit(_x52, _x53) {
...
// 并行竞速
const raceApis = /*#__PURE__*/function () {
  var _ref30 = _asyncToGenerator(function* (apis, info, quality) {
    if (!apis || apis.length === 0) return null;
    return new Promise(resolve => {
      let remaining = apis.length;
      let settled = false;
      apis.forEach(/*#__PURE__*/function () {
        var _ref31 = _asyncToGenerator(function* (api) {
          try {
            const url = yield api.fn(info, quality);
            if (url && isValidAudioUrl(url)) {
              if (!settled) {
                settled = true;
                resolve({
  ...
  return function raceApis(_x54, _x55, _x56) {
    return _ref30.apply(this, arguments);
  };
}();

// ========== 聚合获取URL ==========
const getMusicUrl = /*#__PURE__*/function () {
  var _ref32 = _asyncToGenerator(function* (source, musicInfo, quality) {
    ...
    if (source === 'mg') {
      ...
      for (let gi = 0; gi < groups.length; gi++) {
        ...
        const result = yield raceApis(group, musicInfo, quality);
    ...
    for (let qi = 0; qi < fallbackChain.length; qi++) {
      ...
      for (let gi = 0; gi < groups.length; gi++) {
        ...
        const result = yield raceApis(group, musicInfo, currentQ);
        if (result && result.url) {
  ...
  return function getMusicUrl(_x58, _x59, _x60) {
    return _ref32.apply(this, arguments);
  };
}();

// ========== 歌词 ==========
const getLyric = /*#__PURE__*/function () {
  var _ref33 = _asyncToGenerator(function* (source, musicInfo) {
    ...
    try {
      if (source === 'wy') {
        ...
        const data = yield httpPost(`${API.localWy.base}/lyric`, {
          id: id,
          cookie: API.neteaseCookie
        });
        ...
        const data = yield httpGet(`${API.localQQ.base}/song/lyric`, {
          mid: id,
          decode: 1
        });
        if (data && data.code === 0 && data.data) {
  ...
  return function getLyric(_x61, _x62) {
    return _ref33.apply(this, arguments);
  };
}();

// ========== 封面 ==========
const getPic = /*#__PURE__*/function () {
  var _ref34 = _asyncToGenerator(function* (source, musicInfo) {
    const id = getId(musicInfo);
    if (!id) {
      log.w('封面: 无歌曲ID');
      return '';
    ...
    try {
      if (source === 'wy') {
        ...
        const data = yield httpPost(`${API.localWy.base}/song/detail`, {
          ids: id,
          cookie: API.neteaseCookie
        });
        ...
        const data = yield httpGet(`${API.localQQ.base}/song/detail`, {
          mids: id
        });
  ...
  return function getPic(_x63, _x64) {
...
on(EVENT_NAMES.request, ({
  ...
  switch (action) {
    case 'musicUrl':
      return getMusicUrl(source, info.musicInfo, info.type).then(url => {
