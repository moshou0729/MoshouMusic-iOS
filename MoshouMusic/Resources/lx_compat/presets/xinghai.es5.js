...
function safeParseBody(body) {
  ...
  try {
    if (isBuffer(body)) {
      if (globalThis.lx?.utils?.buffer?.bufToString) body = globalThis.lx.utils.buffer.bufToString(body, 'utf-8');else if (typeof Buffer !== 'undefined') body = Buffer.from(body).toString('utf-8');else body = String(body);
    }
  } catch (e) {}
  if (typeof body === 'string') {
    const trimmed = body.trim();
...
function buildClientHeader() {
  ...
  try {
    ...
    if (p.includes('android')) deviceType = 'Android';else if (p.includes('ios')) deviceType = 'iOS';else if (p.includes('win')) deviceType = 'Windows';else if (p.includes('mac')) deviceType = 'macOS';else if (p.includes('linux')) deviceType = 'Linux';
  } catch (e) {}
...
function _fetchIp() {
  _fetchIp = _asyncToGenerator(function* () {
    try {
      const r = yield httpFetch(buildUrl('primary', 'ip'), {
        timeout: 3000
      });
      if (r.body?.ip) {
        userIp = r.body.ip;
...
function getWyGDUrl(_x, _x2) {
  return _getWyGDUrl.apply(this, arguments);
}
function _getWyGDUrl() {
  _getWyGDUrl = _asyncToGenerator(function* (id, q) {
    const brMap = {
      '128k': '128',
      '192k': '192',
      '320k': '320',
    ...
    const resp = yield httpFetch(url, {
      headers: {
...
function getWyVipUrl(_x3, _x4) {
  return _getWyVipUrl.apply(this, arguments);
}
function _getWyVipUrl() {
  _getWyVipUrl = _asyncToGenerator(function* (id, q) {
...
function getUrlFromBackend(_x5, _x6, _x7) {
  return _getUrlFromBackend.apply(this, arguments);
}
function _getUrlFromBackend() {
  _getUrlFromBackend = _asyncToGenerator(function* (source, musicInfo, quality) {
...
function fetchMusicUrl(_x8, _x9, _x10) {
  return _fetchMusicUrl.apply(this, arguments);
} // -------------------- 初始化逻辑 --------------------
function _fetchMusicUrl() {
  _fetchMusicUrl = _asyncToGenerator(function* (source, musicInfo, quality) {
    ...
    if (source === 'wy') {
      if (NETEASE_VIP_QUALITY_SET.has(actualQuality)) {
        try {
          result = yield getWyVipUrl(id, actualQuality);
        } catch (e) {}
      }
      if (!result.url) result = yield getWyGDUrl(id, actualQuality);
    } else {
      result = yield getUrlFromBackend(source, musicInfo, actualQuality);
    }
    extraCache.set(id, {
      lyric: result.lyric,
      cover: result.cover
...
function _checkUpdate() {
  _checkUpdate = _asyncToGenerator(function* () {
    const versionUrls = [buildUrl('primary', 'version') + '?ver=' + encodeURIComponent(SCRIPT_VERSION), buildUrl('fallback', 'version') + '?ver=' + encodeURIComponent(SCRIPT_VERSION)];
    try {
      const resp = yield Promise.any(versionUrls.map(u => httpFetch(u, {
        timeout: 5000
      })));
...
on(EVENT_NAMES.request, /*#__PURE__*/function () {
  var _ref = _asyncToGenerator(function* ({
    ...
    if (action === 'lyric') return cached?.lyric ? {
      lyric: cached.lyric,
      tlyric: ''
    } : null;
